...
"""Explicit full-checkout Phase 0 validation.

This file intentionally lives under ``tests/integration`` because the Hermes
parallel unit-test runner applies a five-minute per-file cap and runs many files
against one shared checkout. Run this test directly when inventory code,
plugin roots, or registry contracts change::

    pytest tests/integration/platform_inventory/test_repository_integration.py -q
"""
